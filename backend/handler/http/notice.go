...
func NewNoticeHandler(conn *sql.DB) *Ntc {
	//log.Printf("Passing user handler an sql connection to get Resources struct in handler/http/user.Resources")
	return &Ntc{
		repo: user.NewNoticeRepository(conn),
	}
}

func (user *Ntc) GetHTTPHandler() []*handler.HTTPHandler {
	log.Printf("Task Http Handler Redirecting to object based on path..")
	return []*handler.HTTPHandler{
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "notice/{id}", Func: user.GetByID},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPost, Path: "notice", Func: user.Create},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPut, Path: "notice/{id}", Func: user.Update},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodDelete, Path: "notice/{id}", Func: user.Delete},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "notice", Func: user.GetAll},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPatch, Path: "notice/{id}", Func: user.Update},
	}
}
func (user *Ntc) GetByID(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	var usr interface{}
...
func (user *Ntc) Create(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	//log.Printf("Creating usr object defined in model")
	var usr model.Notice
...
func (user *Ntc) Update(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	...
	for {
		...
		usr = iUsr.(model.Notice)
		break
	}
	handler.WriteJSONResponse(w, r, usr, http.StatusOK, err)
}

func (user *Ntc) Delete(w http.ResponseWriter, r *http.Request) {
	var payload string
	id, err := strconv.ParseInt(chi.URLParam(r, "id"), 10, 64)
	for {
		if nil != err {
...
func (user *Ntc) GetAll(w http.ResponseWriter, r *http.Request) {
