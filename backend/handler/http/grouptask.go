...
func NewGroupTaskHandler(conn *sql.DB) *GroupTask {
	//log.Printf("Passing user handler an sql connection to get UserGroup struct in handler/http/user.UserGroup")
	return &GroupTask{
		repo: user.NewGroupTaskRepository(conn),
	}
}

func (user *GroupTask) GetHTTPHandler() []*handler.HTTPHandler {
	log.Printf("Http Handler Redirecting to object based on path ..guser_groups.")
	return []*handler.HTTPHandler{
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "group_task/{id}", Func: user.GetByID},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPost, Path: "group_task", Func: user.Create},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPut, Path: "group_task/{id}", Func: user.Update},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodDelete, Path: "group_task/{id}", Func: user.Delete},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "group_task", Func: user.GetAll},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPatch, Path: "group_task/{id}", Func: user.Update},
	}
}
func (user *GroupTask) GetByID(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	var usr interface{}
...
func (user *GroupTask) Create(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	//log.Printf("Creating usr object defined in model")
	var usr model.GroupTask
...
func (user *GroupTask) Update(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	...
	for {
		...
		usr = iUsr.(model.GroupTask)
		break
	}
	handler.WriteJSONResponse(w, r, usr, http.StatusOK, err)
}

func (user *GroupTask) Delete(w http.ResponseWriter, r *http.Request) {
	var payload string
	id, err := strconv.ParseInt(chi.URLParam(r, "id"), 10, 64)
	for {
		if nil != err {
...
func (user *GroupTask) GetAll(w http.ResponseWriter, r *http.Request) {
