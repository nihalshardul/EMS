...
func NewLeavesHandler(conn *sql.DB) *Leave {
	//log.Printf("Passing user handler an sql connection to get Resources struct in handler/http/user.Resources")
	return &Leave{
		repo: user.NewLeavesRepository(conn),
	}
}

func (user *Leave) GetHTTPHandler() []*handler.HTTPHandler {
	log.Printf("Task Http Handler Redirecting to object based on path..")
	return []*handler.HTTPHandler{
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "leaves/{id}", Func: user.GetByID},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPost, Path: "leaves", Func: user.Create},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPut, Path: "leaves/{id}", Func: user.Update},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodDelete, Path: "leaves/{id}", Func: user.Delete},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "leaves", Func: user.GetAll},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPatch, Path: "leaves/{id}", Func: user.Update},
	}
}
func (user *Leave) GetByID(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	var usr interface{}
...
func (user *Leave) Create(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	//log.Printf("Creating usr object defined in model")
	var usr model.Leaves
...
func (user *Leave) Update(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
	...
	for {
		...
		usr = iUsr.(model.Leaves)
		break
	}
	handler.WriteJSONResponse(w, r, usr, http.StatusOK, err)
}

func (user *Leave) Delete(w http.ResponseWriter, r *http.Request) {
	var payload string
	id, err := strconv.ParseInt(chi.URLParam(r, "id"), 10, 64)
	for {
		if nil != err {
...
func (user *Leave) GetAll(w http.ResponseWriter, r *http.Request) {
