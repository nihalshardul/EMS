...
import (
	...
	httpHandler "github.com/backend/handler/http"
)
...
func init() {
	...
	handlers = []handler.IHTTPHandler{
		...
		httpHandler.NewNoticeHandler(dbConn),
		httpHandler.NewAttendanceHandler(dbConn),
		httpHandler.NewLeavesHandler(dbConn),
		httpHandler.NewQueryHandler(dbConn),
		httpHandler.NewAnsToQueryHandler(dbConn),
		httpHandler.NewEmpGroupHandler(dbConn),
		httpHandler.NewGroupTaskHandler(dbConn),
		httpHandler.NewLoginHandler(dbConn),
	}
}
